...
const state = {
  ...
  data: {
    ...
    currency: "ngn", budgetBand: "", budgetCustom: "", duration: "",
    postingFrequency: "",
...
buildRadioGroup(document.getElementById("currency-group"), CURRENCY_OPTIONS, "currency", (val) => {
  state.data.currency = val;
  updateBudgetVisibility();
});
buildRadioGroup(document.getElementById("budget-group"), BUDGET_OPTIONS, "budgetBand", (val) => {
  state.data.budgetBand = val;
});
...
function updateBudgetVisibility() {
  const bandsWrap = document.getElementById("budget-bands-wrap");
  const customWrap = document.getElementById("budget-custom-wrap");
  const isNgn = state.data.currency === "ngn";
  bandsWrap.classList.toggle("is-visible", isNgn);
  customWrap.classList.toggle("is-visible", !isNgn);
}

// Default the Naira option to selected on load, matching state.data.currency's default.
document.getElementById(`currency-${state.data.currency}`).checked = true;
updateBudgetVisibility();

document.getElementById("budget-custom-input").addEventListener("input", (e) => {
  state.data.budgetCustom = e.target.value.trim();
});
...
function resolveBudgetLabel(data) {
  if (data.currency === "ngn") {
    return data.budgetBand ? labelFor(BUDGET_OPTIONS, data.budgetBand) : "";
  }
  return data.budgetCustom ? data.budgetCustom : "";
}
...
function generateVerdict(data) {
  ...
  const budgetLabel = resolveBudgetLabel(data);
  const scopeLine = (budgetLabel || data.duration)
    ? `Scoped against ${budgetLabel ? budgetLabel.toLowerCase() : "a flexible budget"}, over ${data.duration ? labelFor(DURATION_OPTIONS, data.duration).toLowerCase() : "a timeline we'll set together"}.`
    : "";
...
function buildPayload() {
  return {
    ...
    currency: labelFor(CURRENCY_OPTIONS, state.data.currency),
    budget: resolveBudgetLabel(state.data),
